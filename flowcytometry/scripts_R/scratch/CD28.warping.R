...
## file paths: previously-warped samples
...
input.set <- fset.compensate(read.flowSet(c(tphe.warped.files[grep(batch.name, tphe.warped.files)], target.file), transformation = FALSE)) # read in flowSet; compensate
input.set <- transform.set(input.set) # transform using 'trans.obj'
