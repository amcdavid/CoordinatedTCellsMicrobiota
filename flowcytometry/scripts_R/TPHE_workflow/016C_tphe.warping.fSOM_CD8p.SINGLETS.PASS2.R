...
##
warp.parameters <- readRDS("./results_R/FCS.transforms/TPHE/TPHE.CD8.warp.parameters.rds") # channels-of-interest
warp.markers <- c("CD122", "GRZB")
...
## file paths
fsom.files <- list.files("./data_modified/TPHE_warping/EXPERIMENTS/", 
                         recursive = TRUE, full.names = TRUE, pattern = "CD8p.SINGLETS_.fcs")
...
input.set.warped.inverse.decomp.mod <- fsApply(input.set.warped.inverse.decomp, function (i) {
  ...
  i@description$`WARPED` <- "fda_WARPED_fSOM_CD8p.SINGLETS"
...
outDir = file.path("./data_modified/TPHE_warping/EXPERIMENTS/", batch.name, "PERFORIN.CD57.CD28.FOXP3.CD197.CD185.CD31.CD122.GRZB_warped_CD8p")
