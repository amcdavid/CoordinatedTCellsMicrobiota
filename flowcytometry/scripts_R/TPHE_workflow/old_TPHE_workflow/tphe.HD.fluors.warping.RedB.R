...
channel.name <- "Red B" # set
marker.name <- "CD185" # set
...
sub.set.warped <- warpSet(fset.downsample(input.set.transformed.rangefix, 10000)[, channel], c(channel), monwrd = FALSE, peakNr = 1, clipRange = 0.01, bwFac = 2, 
...
input.set.transformed.rangefix.warped <- warpSet(input.set.transformed.rangefix, c(channel), monwrd = FALSE, peakNr = 1, clipRange = 0, bwFac = 2, 
                                                 target = sampleNames(input.set.transformed.rangefix[grep(target, sampleNames(input.set.transformed.rangefix))]))
...
input.set.transformed.rangefix.warped.inverse.decomp.mod <- fsApply(input.set.transformed.rangefix.warped.inverse.decomp, function (i) {
  i@description$`WARPED CHANNELS` <- "FSC.AHW_SSC.AH_BA_VG_VE_VD_VA_RB_RA_GE_GD_GA"
  i
})
