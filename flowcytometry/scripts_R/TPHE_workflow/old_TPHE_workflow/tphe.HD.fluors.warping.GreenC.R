...
channel.name <- "Green C" # set
marker.name <- "CD28" # set

## subset data includes only positive so use original data to optimize transforms
...
densityplot(~ ., fset.downsample(input.set.transformed.rangefix, 10000)[, channel], filter = curv1Filter(channel, bwFac = bwFac.set))
sub.set.warped <- warpSet(fset.downsample(input.set.transformed.rangefix, 10000)[, channel], c(channel), monwrd = FALSE, peakNr = 1, clipRange = 0.01, bwFac = bwFac.set, 
...
input.set.transformed.rangefix.warped <- warpSet(input.set.transformed.rangefix, c(channel), monwrd = FALSE, peakNr = 1, clipRange = 0.01, bwFac = bwFac.set, 
                                                 target = sampleNames(input.set.transformed.rangefix[grep(target, sampleNames(input.set.transformed.rangefix))]))
densityplot(~ ., input.set.transformed.rangefix.warped[, channel], filter = curv1Filter(channel, bwFac = bwFac.set))
...
input.set.transformed.rangefix.warped.inverse.decomp.mod <- fsApply(input.set.transformed.rangefix.warped.inverse.decomp, function (i) {
  i@description$`WARPED CHANNELS` <- "FSC.AHW_SSC.AH_BA_VG_VE_VD_VC_VB_VA_RB_RA_GE_GC_GD_GA"
  i
})
