...
cst_assoc = has_cst %>% group_by(Renamed_CST) %>% do({
    data = .
    full = glm(has_cst ~ preterm_weeks + mode_delivery + scale(birth_wt_gms) + scale(auc14) + preg_antibiotics + milk_months + perinatal_milk + n_antibiotics_pre + n_antibiotics_discharge + Gender, family = 'binomial', data = data)
...
Controlling for `r filter(cst_assoc, name == 'full') %>% slice(1) %>% pull(model) %>% .[[1]] %>% formula %>% as.character() %>% .[3]`
